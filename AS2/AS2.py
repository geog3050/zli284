...
Pok̩mon, also known as Pocket Monsters in Japan, is a Japanese media franchise managed by
The Pok̩mon Company, a company founded by Nintendo, Game Freak, and Creatures. The franchise
was created by Satoshi Tajiri in 1995, and is centered on fictional creatures called "Pok̩mon",
which humans, known as Pok̩mon Trainers, catch and train to battle each other for sport.
Works within the franchise are set in the Pok̩mon universe. The English slogan for the franchise
    ...
    try: 
        with open(filename, 'r') as read_obj:
            ...
            for row in csv_reader:
                ...
                processLine = []
                processLine.extend(row[:2])
                processLine.extend([float(x) for x in row[2:]])
                participants.append(processLine)
...
def attack_multiplier(attacker_type, defender_type):
    ...
    # 6. Else, return 1.0
    ######################
    if attacker_type in ['Water', 'Fire', 'Electric', 'Ground', 'Grass'] and defender_type in ['Water', 'Fire', 'Electric', 'Ground', 'Grass']:
        dic = {'WaterFire': 2.5, 'ElectricWater': 1.3, 'GroundElectric': 2.0, 'FireGrass': 3.0, 'GrassWater': 1.5}    
        if attacker_type+defender_type in dic:
            return dic[attacker_type+defender_type]
        else: 
            return 1.0
    else: 
        print("Check the type of the two inputs.")
        return
...
def fight(participant1, participant2, first2attack):
    ...
    winner = 1
    attack = first2attack
    p1 = copy.deepcopy(participant1)
    p2 = copy.deepcopy(participant2)
    
    while p1[2]>0 and p2[2]>0:
        if attack == 1:
            p2[2] -= attack_multiplier(p1[1], p2[1])*p1[3]
            attack = 2
        else:
            p1[2] -= attack_multiplier(p2[1], p1[1])*p2[3]
            attack = 1
        rounds += 1
...
def tournament(participants):
    ...
    # 3. Return WINS list
    ######################
    wins = [0 for x in range(len(participants))]
    for index1 in range(len(participants)):
        for index2 in range(len(participants)):
            if index1 != index2:
                if fight(participants[index1], participants[index2], 1)[0] == 1:
                    wins[index1] += 1
                if fight(participants[index1], participants[index2], 2)[0] == 1:
                    wins[index1] += 1 
    return wins
